...
public class IngreController {
	...
	@RequestMapping("reMenu") //메뉴 등록 뷰에서 등록 버튼 클릭시 
	public ModelAndView reMenu(@Valid Menu menu,BindingResult bindResult, HttpServletRequest request) {
	...
	@RequestMapping("addMenu") //메뉴리스트에서 등록버튼 클릭시
	...
	@RequestMapping("madd") //메뉴리스트에서 재료등록 클릭시
	public ModelAndView madd(IngrePerMenu IPM,BindingResult bindResult, HttpServletRequest request) {
		ModelAndView mav = new ModelAndView();
		List<Ingre> ingrelist = service.ingrelist();
		List<IngrePerMenu> IPMlist = service.IPMlist();
		mav.addObject(new Menu());
//		for(Ingre ig : ingrelist) {
//			List<IngrePerMenu> IPMlist = service.IPMlist(ig.getIngreNo());
//			for(IngrePerMenu IPMl : IPMlist) { 
//				Ingre ingre = service.getIngreByNo(IPMl.getIngreNo());
//				IPMl.setIngre(ingre);
//			}
//			ig.setIPMlist(IPMlist); 
//		}
		System.out.println(IPMlist);
		mav.addObject("ingrelist",ingrelist);
		mav.addObject("IPMlist",IPMlist);
		service.IPMInsert(IPM,request);
		mav.setViewName("redirect:/ingre/menuList.shop");
		return mav;
	}
	@RequestMapping("relist") 
	public ModelAndView relist(IngrePerMenu IPM,BindingResult bindResult, HttpServletRequest request) {
		ModelAndView mav = new ModelAndView();
		List<Ingre> ingrelist = service.ingrelist();
		List<IngrePerMenu> IPMlist = service.IPMlist();
		mav.addObject(new Menu());
