...
<table>
	<caption style="font-size:25px;">1:1상담</caption>
	<tr><td colspan="5" style="border-top:0">
		<form action="consult.shop" method="post" name="searchform">
	...
	<c:if test="${listcount > 0}">
		...
		<tr><td colspan="5">
				<c:if test="${pageNum > 1}"><a href="consult.shop?pageNum=${pageNum - 1}&searchtype=${param.searchtype}&searchcontent=${param.searchcontent}">[이전]</a></c:if>
				<c:if test="${pageNum <= 1}">[이전]</c:if>
				<c:forEach var="a" begin="${startpage}" end="${endpage}">
					<c:if test="${a == pageNum}">[${a}]</c:if>
					<c:if test="${a != pageNum}"><a href="consult.shop?pageNum=${a}&searchtype=${param.searchtype}&searchcontent=${param.searchcontent}">[${a}]</a></c:if>
				</c:forEach>
				<c:if test="${pageNum < maxpage}"><a href="consult.shop?pageNum=${pageNum + 1}&searchtype=${param.searchtype}&searchcontent=${param.searchcontent}">[다음]</a></c:if>
